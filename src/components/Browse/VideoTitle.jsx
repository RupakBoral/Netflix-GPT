...
const VideoTitle = ({title, overview, vote_average}) => {
    ...
    return (
        <div className='z-40 text-white sm:space-y-2 lg:space-y-6 absolute text-opacity-85 top-[25%] md:top-[25%] pl-8 md:pl-16 overflow-x-hidden w-full'>
            <section className='w-1/2 md:w-1/3 space-y-4 p-5 lg::p-8 rounded-xl'>
                <h1 className='md:text-3xl text-xl font-bold'>{title}</h1>
                <h3 className='text-xl font-medium hidden lg:block'>{overview}</h3>
                <p className='text-xl font-bold text-yellow-500 hidden lg:block text-opacity-95'>Rating: {vote_average.toFixed(1)}</p>
            </section>
            <section className='flex gap-2 lg:gap-8 w-1/2 lg:w-1/3'>
                <button 
                    onMouseEnter={() => setisHoveredPlay(true)}
                    onMouseLeave={() => setisHoveredPlay(false)}
                    className='flex gap-2 items-center justify-center py-2 lg:py-5 w-1/2 text-opacity-85 bg-opacity-85 rounded-md bg-gray-100 font-bold text-2xl text-red-600 hover:text-gray-100 hover:bg-red-600 transition-all duration-1000'>
                ...
                <button 
                    ...
                    <span className='lg:block hidden'>{lang[currentLang].MoreInfo}</span>
                </button>
            </section>
        </div>
    );
