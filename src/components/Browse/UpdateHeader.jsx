...
const UpdateHeader = () => {
  ...
  return (
    <div className="flex z-40 bg-transparent md:gap-6 sm:gap-4 gap-2 items-center absolute right-10 top-6 md:top-16">
      <select className="md:px-4 md:py-2 sm:px-2 sm:py-1 my-auto rounded-md text-gray-100 bg-transparent  font-semibold cursor-pointer text-base sm:text-xl ms:text-xl">
        {LANGUAGE.map((lang) => (
          <option
            key={lang.identifier}
            ...
            className="text-gray-700 bg-zinc-300"
          >
            {lang.name}
          </option>
        ))}
      ...
      <img
        className="lg:w-12 lg:h-12 md:w-10 md:h-10 w-8 h-8 rounded-full cursor-pointer my-auto"
        src={user_logo}
        alt="user"
      />
      <button
