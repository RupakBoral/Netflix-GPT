...
const SearchBar = () => {
  ...
  const handleGPTSearch = async () => {
    if (BarPosition === false) setBarPosition(true);
    const systemPrompt =
      "You are a Movie Recommendation AI Agent. Respond only in JSON. Provide an object with two keys: 'Movies' (array of atmost 5 movie names) and 'Shows' (array of at most 5 and show names).";
    ...
    if (response) {
      const PromiseArray = response.Movies.map((movie) =>
        SearchResultTmdb(movie)
      );
    ...
    >
      <div className="w-full flex justify-between">
        ...
        <button
          ...
          className="text-2xl font-medium text-red-300"
        >
          {lang[currentLang].SearchWithGPt}⭐
        </button>
      </div>
      <div className="w-full mx-auto">
        <div
          className={`flex ${
            searchGpt ? "translate-x-1/2" : "translate-x-0"
